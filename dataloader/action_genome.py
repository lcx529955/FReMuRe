...
#             im = im[:, :, ::-1] # rgb -> bgr
        ...
        im_info = torch.from_numpy(im_info).repeat(blob.shape[0], 1)  # 按照帧数重复
        img_tensor = torch.from_numpy(blob)  # (29, 1067, 600, 3), (n, w, h, 3)
        img_tensor = img_tensor.permute(0, 3, 1, 2)  # (29, 3, 1067, 600), (n, 3, w, h)
    ...
    def __len__(self):
        return len(self.video_list)  # 7584
