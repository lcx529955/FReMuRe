...
from lib.gmm_heads_plus import *
from lib.bayesian_heads import *
from lib.BayesianHeadVMF import *
...
class ObjectClassifier(nn.Module):
    """
    Module for computing the object contexts and edge contexts， 用于计算对象上下文和边缘上下文的模块
    """
...
class FReMuRe(nn.Module):
    ...
    def __init__(self, mode='sgdet',attention_class_num=None, spatial_class_num=None,
                 ...
                 obj_head='gmm', rel_head='gmm', K=None, tracking=None, freq_attention=None, freq_spatial=None, freq_contact=None):
        ...
        self.freq_attention = freq_attention
        self.freq_spatial = freq_spatial
        self.freq_contact = freq_contact
        ...
        embed_vecs = obj_edge_vectors(obj_classes, wv_type='glove.6B', wv_dir='data/', wv_dim=200)  # (37, 200), 37种object，对应200维的向量
        self.obj_embed = nn.Embedding(len(obj_classes), 200)  # 默认使用的是均匀分布的随机初始化，范围大约是 [-1, 1]
        self.obj_embed.weight.data = embed_vecs.clone()  # 用embed_vecs的值替换前面随机生成的值
        ...
        elif rel_head == 'bayesian':
            self.a_rel_compress = BayesianHead(hid_dim=1936, num_classes=self.attention_class_num, rel_type='attention', sample_n=6)
            self.s_rel_compress = BayesianHead(hid_dim=1936, num_classes=self.spatial_class_num, rel_type='spatial', sample_n=6)
            self.c_rel_compress = BayesianHead(hid_dim=1936, num_classes=self.contact_class_num, rel_type='contact', sample_n=6)
        elif rel_head == 'bayesian_vmf':
            self.a_rel_compress = BayesianHeadVMF(hid_dim=1936, num_classes=self.attention_class_num, rel_type='attention', sample_n=6)
            self.s_rel_compress = BayesianHeadVMF(hid_dim=1936, num_classes=self.spatial_class_num, rel_type='spatial', sample_n=6)
            self.c_rel_compress = BayesianHeadVMF(hid_dim=1936, num_classes=self.contact_class_num, rel_type='contact', sample_n=6)
        elif rel_head == 'gmm_plus':
            self.a_rel_compress = GMMHeadPlus(hid_dim=1936, num_classes=self.attention_class_num, rel_type='attention', k=self.GMM_K)
            self.s_rel_compress = GMMHeadPlus(hid_dim=1936, num_classes=self.spatial_class_num, rel_type='spatial', k=self.GMM_K)
            self.c_rel_compress = GMMHeadPlus(hid_dim=1936, num_classes=self.contact_class_num, rel_type='contact', k=self.GMM_K)

        else:
            self.a_rel_compress = nn.Linear(1936, int(self.attention_class_num))
            self.s_rel_compress = nn.Linear(1936, int(self.spatial_class_num))
            self.c_rel_compress = nn.Linear(1936, int(self.contact_class_num))
    ...
    def forward(self, entry, phase='train',unc=False):
        ...
        rel_features = torch.cat((x_visual, x_semantic), dim=1)  #1936

        # Spatial-Temporal Transformer，PEG部分
        if self.freq_attention is not None:
            global_attention_output,rel_attention_features,mem_attention_features, _, _ = self.glocal_transformer(
                features=rel_features, im_idx=entry['im_idx'], memory=self.rel_memory, rel_type='attention', freq_tensor=self.freq_attention)
            global_spatial_output, rel_spatial_features, mem_spatial_features, _, _ = self.glocal_transformer(
                features=rel_features, im_idx=entry['im_idx'], memory=self.rel_memory, rel_type='spatial', freq_tensor=self.freq_spatial)
            global_contact_output, rel_contact_features, mem_contact_features, _, _ = self.glocal_transformer(
                features=rel_features, im_idx=entry['im_idx'], memory=self.rel_memory, rel_type='contact', freq_tensor=self.freq_contact)

            entry["attention_features"] = rel_attention_features
            entry["spatial_features"] = rel_spatial_features
            entry["contacting_features"] = rel_contact_features
            entry["attention_mem_features"] = mem_attention_features
            entry["spatial_mem_features"] = mem_spatial_features
            entry["contacting_mem_features"] = mem_contact_features
            entry["freq_attention"] = self.freq_attention
            entry["freq_spatial"] = self.freq_spatial
            entry["freq_contact"] = self.freq_contact

            if self.rel_head == 'gmm' or self.rel_head == 'gmm_plus' or self.rel_head == 'bayesian' or self.rel_head == 'bayesian_vmf':
                if not unc:
                    entry["attention_distribution"] = self.a_rel_compress(global_attention_output,phase,unc)
                    entry["spatial_distribution"] = self.s_rel_compress(global_spatial_output,phase,unc)
                    entry["contacting_distribution"] = self.c_rel_compress(global_contact_output,phase,unc)
                else:
                    entry["attention_al_uc"], entry["attention_ep_uc"] = self.a_rel_compress(global_attention_output,phase,unc)
                    entry["spatial_al_uc"], entry["spatial_ep_uc"] = self.s_rel_compress(global_spatial_output,phase,unc)
                    entry["contacting_al_uc"], entry["contacting_ep_uc"] = self.c_rel_compress(global_contact_output,phase,unc)
            else:
                entry["attention_distribution"] = self.a_rel_compress(global_attention_output)
                if phase == 'test':
                    entry["attention_distribution"] = entry["attention_distribution"].softmax(-1)
                entry["spatial_distribution"] = self.s_rel_compress(global_spatial_output)
                entry["contacting_distribution"] = self.c_rel_compress(global_contact_output)
