...
def obj_edge_vectors(names, wv_type='glove.6B', wv_dir=None, wv_dim=300):
    ...
    # wv_dict是一个词典，将词映射到词向量的索引，40万个词
    # wv_arr是一个包含所有词向量的数组
    # wv_size是词向量的维度

    vectors = torch.Tensor(len(names), wv_dim)
    vectors.normal_(0, 1)  # 使符合（0，1）正态分布
    ...
    for i, token in enumerate(names):
        ...
        if wv_index is not None:
            ...
            lw_token = sorted(token.split('__'), key=lambda x: len(x), reverse=True)[0]
    ...
    return vectors  # (37, 200), 37种object，对应200维的向量
