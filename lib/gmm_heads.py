...
class GMM_head(nn.Module):
    def __init__(self, hid_dim, num_classes, rel_type=None, k=4):
        ...
        self.heads = nn.ModuleDict()  # 是用来存储多个子模块的容器，它提供了类似字典的存储方式，但同时也确保这些子模块会参与模型的训练过程。
        self.rel_type = rel_type  # attention
        for i in range(k):
        ...
        if rel_type == 'attention' or rel_type is None:
            self.activation = nn.Softmax(-1)  # 表示对最后一个维度进行 Softmax 运算，通常是对类别维度进行归一化，使得输出符合概率分布的特性。
        else:
