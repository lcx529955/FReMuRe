...
class detector(nn.Module):
    ...
    def __init__(self, train, object_classes, use_SUPPLY, mode='predcls'):
        ...
        checkpoint_path = osp.join(project_root, 'fasterRCNN', 'models', 'faster_rcnn_ag.pth')
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, gt_annotation, im_all):
        ...
        if self.mode == 'sgdet':
            ...
            # how many bboxes we have in video, 每一帧的都加起来
            bbox_num = 0
            ...
            bbox_idx = 0
            for i, j in enumerate(gt_annotation):  # j是每帧的label
                for m in j:  # m是每个object
                    if 'person_bbox' in m.keys():
                        FINAL_BBOXES[bbox_idx, 1:] = torch.from_numpy(m['person_bbox'][0])
                        FINAL_BBOXES[bbox_idx, 0] = i  # 第几帧
                        ...
                        FINAL_BBOXES[bbox_idx, 0] = i  # 第几帧
                        FINAL_LABELS[bbox_idx] = m['class']  # object 编号
                        im_idx.append(i)  # 第几帧
